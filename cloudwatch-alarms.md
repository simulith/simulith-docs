...
Topics must exist locally (`aws sns create-topic`, Terraform `aws_sns_topic`, or default seed **`demo-alarm`**). Messages are stored in SQLite for inspection; optional **Subscribe** (Lambda/SQS) fan-out on `Publish`. No email/SMS.
