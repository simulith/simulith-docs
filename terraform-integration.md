...
Runnable modules: [`examples/terraform/ssm/`](examples/terraform/ssm/) (minimal `/app/tf-demo/*`) · [`examples/terraform/ssm/parameters/`](examples/terraform/ssm/parameters/) (26× `/SIMULITH/DEV/*` locally). **DeleteParameter** is implemented — `terraform destroy` removes managed parameters. Use **`-parallelism=1`** on apply and destroy (SQLite). Console SSM browse paginates `GetParametersByPath` (10 per page).
...
| [`ssm/parameters/`](examples/terraform/ssm/parameters/) | Green | Green | 26 params; `dev.tfvars` / `dev.aws.tfvars`; `-parallelism=1` |
...
| [`multi-root-green-path/`](examples/terraform/multi-root-green-path/) | Green | Green | Production chain: bootstrap → **01-vpc** → **02-subnets** → **03-secrets** → **04-postgresdb** → **05-proxydb** → **06-ses** → **07-cognito** → **08-parameters** → **09-dynamodb** via S3 remote state — `-parallelism=1` |
...
| [`proxydb/`](examples/terraform/proxydb/) | Green | Green | RDS Proxy + IAM; outputs `rds_proxy_*` — `-parallelism=1`; **Docker required** |
...
| Lambda Java (`java17`) | — | [`aws-cli/lambda-java/`](examples/aws-cli/lambda-java/) | Compile `App.class` with `javac`; handler `App::handleRequest` |
...
Do not copy a customer Terraform tree into this repo. Remaining AWS gaps from that discovery: packet NAT/IGW/NACL data plane (product out of scope). Interface endpoints, NAT Gateway, and Network ACL **metadata** shipped. KMS `enable_key_rotation` shipped. RDS `ModifyDBParameterGroup` shipped. RDS `ModifyDBInstance` backup/maintenance metadata shipped. RDS `ModifyDBProxy` idle/pool metadata shipped. IAM inline role policies (`PutRolePolicy` / `aws_iam_role_policy`) shipped. Cognito pool MFA config (`SetUserPoolMfaConfig`) shipped. CloudFront managed cache policies (`ListCachePolicies` / `GetCachePolicy`) shipped. CloudFront distribution tags / IPv6 / SPA errors shipped. Cognito user pool tags (`TagResource` / `ListTagsForResource`) shipped. Cognito app client settings (`UpdateUserPoolClient`) shipped. Cognito custom schema (`AddCustomAttributes` / `SchemaAttributes`) shipped. Cognito `ListUsers` shipped. Cognito Lambda triggers + domain Terraform depth shipped. Cognito PreSignUp Lambda trigger shipped. Parameters root Terraform green path shipped. SES root Terraform green path shipped. Secrets root Terraform green path shipped. Subnets root Terraform green path shipped. Postgresdb root Terraform green path shipped. Proxydb root Terraform green path shipped. Vpc root Terraform green path shipped. Production multi-root Terraform green path shipped. Multi-root chain through **08-parameters** shipped. RDS Proxy endpoint in remote state — `CreateDBProxy` pre-allocates local endpoint for unmodified `parameters/` remote state.
...
| S3 `force_destroy` | **Available** — ListObjectsV2 or ListObjectVersions, then DeleteObjects, then DeleteBucket |
| S3 object version IDs / ACL writes | **ListObjectVersions** + current-object version IDs. Overwrite replaces the current version (no noncurrent history). ACL writes not supported. |
