# Layer 3 - unmodified validator cognito/ apply (standalone root; chains upstream for audit order)
# Requires: VALIDATOR_COGNITO_ROOT + all env vars from run-l3-ses.ps1
...
$Log = Join-Path $ScriptDir "l3-cognito.log"
...
if ([string]::IsNullOrWhiteSpace($CognitoRoot)) {
    throw "Set VALIDATOR_COGNITO_ROOT to the validator cognito/ directory (local checkout only; not committed to simulith)."
}
...
foreach ($p in @($CognitoRoot, $SesRoot, $ProxydbRoot, $PostgresdbRoot, $SecretsRoot, $SubnetsRoot, $VpcRoot, $BootstrapRoot)) {
...
function Invoke-Upstream([string]$UpstreamAction) {
    Write-Log "Upstream $UpstreamAction (via run-l3-ses.ps1)"
    ...
    & powershell -ExecutionPolicy Bypass -File (Join-Path $ScriptDir "run-l3-ses.ps1") -Action $UpstreamAction
    if ($LASTEXITCODE -ne 0) { throw "upstream $UpstreamAction failed (exit $LASTEXITCODE)" }
}

function Invoke-CognitoStep([string]$CognitoAction, [switch]$AllowFailure) {
    $OverrideSrc = Join-Path $ScriptDir "simulith.provider-override-cognito.tf.example"
    $OverrideDst = Join-Path $CognitoRoot "simulith_override.tf"
    Copy-Item $OverrideSrc $OverrideDst -Force
    Write-Log "Copied Cognito/Lambda/IAM provider override to $OverrideDst"
    Write-Log "L3 cognito $CognitoAction start (root=$CognitoRoot)"
    Set-Location $CognitoRoot
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        & terraform init "-backend-config=$Backend" '-input=false' '-reconfigure' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8
        if ($LASTEXITCODE -ne 0) { throw "cognito init failed (exit $LASTEXITCODE)" }
        $tfvars = Join-Path $CognitoRoot "dev.tfvars"
        if (-not (Test-Path $tfvars)) { throw "Missing dev.tfvars: $tfvars" }
        & terraform $CognitoAction "-var-file=$tfvars" '-parallelism=1' '-auto-approve' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8
        if ($LASTEXITCODE -ne 0) {
            if ($AllowFailure) {
                Write-Log "WARN cognito $CognitoAction failed (exit $LASTEXITCODE) - continuing"
            } else {
                throw "cognito $CognitoAction failed (exit $LASTEXITCODE)"
            }
        }
    } finally {
    ...
    if (-not $AllowFailure -or $LASTEXITCODE -eq 0) {
        Write-Log "OK L3 cognito $CognitoAction"
    }
}

$sessionLine = "========== {0} L3 cognito {1} start ==========" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Action
try {
...
Write-Log "L3 cognito $Action start"

switch ($Action) {
    "apply" {
        Invoke-Upstream "apply"
        Invoke-CognitoStep "destroy" -AllowFailure
        Invoke-CognitoStep "apply"
    }
    "destroy" {
        Invoke-CognitoStep "destroy"
...
Write-Log "OK L3 cognito $Action complete"
