# Layer 4 - unmodified validator otp-api deploy (1 function via CreateFunction)
# Requires: VALIDATOR_OTP_API_ROOT, VALIDATOR_LAYER_TRANSVERSAL_ROOT,
#           VALIDATOR_LAYER_EMAIL_ROOT, VALIDATOR_LAYER_OTP_ROOT + Simulith on :4566
# Upstream: layer-transversal + layer-email + layer-otp; auth-api authorizer documented for HTTP
param(
    [ValidateSet("publish", "remove", "full")]
    [string]$Action = "full",
    [ValidateSet("all", "handlingOtp")]
    [string]$Function = "all"
)

$ErrorActionPreference = "Stop"
$ScriptDir = $PSScriptRoot
$Log = Join-Path $ScriptDir "l4-otp-api.log"
$DummyRole = "arn:aws:iam::000000000000:role/r"

$OtpApiRoot = $env:VALIDATOR_OTP_API_ROOT
...
foreach ($pair in @(
    @{ Name = "VALIDATOR_OTP_API_ROOT"; Value = $OtpApiRoot }
    @{ Name = "VALIDATOR_LAYER_TRANSVERSAL_ROOT"; Value = $LayerTransRoot }
    @{ Name = "VALIDATOR_LAYER_EMAIL_ROOT"; Value = $LayerEmailRoot }
...
# dev.json OTP_SERVICE_NAME -> Lambda function name (service-dev-export)
$FunctionSpecs = @(
    @{
        Key = "handlingOtp"
        Service = "otp53rv1c3"
        Export = "handlingOtp"
        CopyItems = @(
            @{ Source = "functions\handlingOtp\handler.js"; Dest = "handler.js" }
            @{ Source = "logs"; Dest = "logs" }
            @{ Source = "services\handlingOtp"; Dest = "handlingOtp" }
        )
    }
)
...
function Invoke-EnsureLayers {
    ...
    if ([string]::IsNullOrWhiteSpace($env:VALIDATOR_LAYER_ENGINE_ROOT)) {
        $env:VALIDATOR_LAYER_ENGINE_ROOT = (Split-Path $LayerOtpRoot -Parent) + "\layer-engine"
    }

    if (-not (Test-LayerPublished "layer-email")) {
        Write-Log "upstream layer-email publish"
        $env:VALIDATOR_LAYER_EMAIL_ROOT = $LayerEmailRoot
        $env:VALIDATOR_LAYER_OTP_ROOT = $LayerOtpRoot
        & powershell -ExecutionPolicy Bypass -File (Join-Path $ScriptDir "run-l4-remaining-layers.ps1") -Action publish -Layer email
        if ($LASTEXITCODE -ne 0) { throw "upstream layer-email publish failed (exit $LASTEXITCODE)" }
    } else {
        Write-Log "layer-email already published (skip rebuild)"
    }

    if (-not (Test-LayerPublished "layer-otp")) {
...
function New-OtpFunctionZip([hashtable]$Spec, [string]$ZipPath) {
    $tmpdir = Join-Path $env:TEMP ("l4-otp-pack-{0}" -f [Guid]::NewGuid().ToString("N"))
    ...
    try {
        foreach ($item in $Spec.CopyItems) {
            $src = Join-Path $OtpApiRoot $item.Source
...
function Invoke-PublishOtpApi {
    Invoke-EnsureLayers
    $transArn = Get-LayerVersionArn "layer-transversal"
    $emailArn = Get-LayerVersionArn "layer-email"
    $otpArn = Get-LayerVersionArn "layer-otp"
    Write-Log "layers transversal=$transArn email=$emailArn otp=$otpArn"
    foreach ($spec in Get-SelectedSpecs) {
        $lambdaName = Get-LambdaName $spec
        $handler = Get-Handler $spec
        $zipPath = Join-Path $env:TEMP ("l4-otp-api-{0}.zip" -f $spec.Key)
        New-OtpFunctionZip $spec $zipPath
        ...
        Invoke-Aws @(
            ...
            "--layers", $transArn, $emailArn, $otpArn,
            "--timeout", "10",
            "--memory-size", "128",
    ...
    Write-Log "OK publish otp-api"
}

function Invoke-RemoveOtpApi {
    foreach ($spec in Get-SelectedSpecs) {
        Remove-LambdaIfExists (Get-LambdaName $spec)
    }
    Write-Log "OK remove otp-api functions"
}

$sessionLine = "========== {0} L4 otp-api {1} ({2}) start ==========" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Action, $Function
"" | Set-Content $Log -Encoding utf8
Add-Content -Path $Log -Value $sessionLine -Encoding utf8
Write-Log "L4 otp-api $Action ($Function) start (root=$OtpApiRoot)"

switch ($Action) {
    "publish" { Invoke-PublishOtpApi }
    "remove"  { Invoke-RemoveOtpApi }
    "full" {
        Invoke-PublishOtpApi
        Invoke-RemoveOtpApi
    }
}

Write-Log "OK L4 otp-api $Action ($Function) complete"
