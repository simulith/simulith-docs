# Layer 3 - unmodified validator dynamodb/ apply (standalone parallel track; chains upstream for audit order)
# Requires: VALIDATOR_DYNAMODB_ROOT + all env vars from run-l3-parameters.ps1
...
$Log = Join-Path $ScriptDir "l3-dynamodb.log"
$Backend = Join-Path $ScriptDir "backend.simulith.hcl"

$DynamodbRoot = $env:VALIDATOR_DYNAMODB_ROOT
...
if ([string]::IsNullOrWhiteSpace($DynamodbRoot)) {
    throw "Set VALIDATOR_DYNAMODB_ROOT to the validator dynamodb/ directory (local checkout only; not committed to simulith)."
}
if ([string]::IsNullOrWhiteSpace($ParametersRoot)) {
    throw "Set VALIDATOR_PARAMETERS_ROOT to the validator parameters/ directory."
}
...
foreach ($p in @($DynamodbRoot, $ParametersRoot, $CognitoRoot, $SesRoot, $ProxydbRoot, $PostgresdbRoot, $SecretsRoot, $SubnetsRoot, $VpcRoot, $BootstrapRoot)) {
...
function Invoke-Upstream([string]$UpstreamAction) {
    Write-Log "Upstream $UpstreamAction (via run-l3-parameters.ps1)"
    $env:VALIDATOR_PARAMETERS_ROOT = $ParametersRoot
    ...
    & powershell -ExecutionPolicy Bypass -File (Join-Path $ScriptDir "run-l3-parameters.ps1") -Action $UpstreamAction
    if ($LASTEXITCODE -ne 0) { throw "upstream $UpstreamAction failed (exit $LASTEXITCODE)" }
}

function Invoke-DynamodbStep([string]$DynamodbAction, [switch]$AllowFailure) {
    $OverrideSrc = Join-Path $ScriptDir "simulith.provider-override-dynamodb.tf.example"
    $OverrideDst = Join-Path $DynamodbRoot "simulith_override.tf"
    Copy-Item $OverrideSrc $OverrideDst -Force
    Write-Log "Copied DynamoDB provider override to $OverrideDst"
    Write-Log "L3 dynamodb $DynamodbAction start (root=$DynamodbRoot)"
    Set-Location $DynamodbRoot
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        & terraform init "-backend-config=$Backend" '-input=false' '-reconfigure' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8
        if ($LASTEXITCODE -ne 0) { throw "dynamodb init failed (exit $LASTEXITCODE)" }
        $tfvars = Join-Path $DynamodbRoot "dev.tfvars"
        if (-not (Test-Path $tfvars)) { throw "Missing dev.tfvars: $tfvars" }
        & terraform $DynamodbAction "-var-file=$tfvars" '-parallelism=1' '-auto-approve' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8
        if ($LASTEXITCODE -ne 0) {
            if ($AllowFailure) {
                Write-Log "WARN dynamodb $DynamodbAction failed (exit $LASTEXITCODE) - continuing"
            } else {
                throw "dynamodb $DynamodbAction failed (exit $LASTEXITCODE)"
            }
        }
    } finally {
    ...
    if (-not $AllowFailure -or $LASTEXITCODE -eq 0) {
        Write-Log "OK L3 dynamodb $DynamodbAction"
    }
}

$sessionLine = "========== {0} L3 dynamodb {1} start ==========" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Action
try {
...
Write-Log "L3 dynamodb $Action start"

switch ($Action) {
    "apply" {
        Invoke-Upstream "apply"
        Invoke-DynamodbStep "destroy" -AllowFailure
        Invoke-DynamodbStep "apply"
    }
    "destroy" {
        Invoke-DynamodbStep "destroy"
...
Write-Log "OK L3 dynamodb $Action complete"
