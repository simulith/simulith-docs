# Layer 3 - unmodified validator web/ apply (standalone parallel track; chains upstream for audit order)
# Requires: VALIDATOR_WEB_ROOT + all env vars from run-l3-dynamodb.ps1
...
$Log = Join-Path $ScriptDir "l3-web.log"
$Backend = Join-Path $ScriptDir "backend.simulith.hcl"

$WebRoot = $env:VALIDATOR_WEB_ROOT
...
if ([string]::IsNullOrWhiteSpace($WebRoot)) {
    throw "Set VALIDATOR_WEB_ROOT to the validator web/ directory (local checkout only; not committed to simulith)."
}
if ([string]::IsNullOrWhiteSpace($DynamodbRoot)) {
    throw "Set VALIDATOR_DYNAMODB_ROOT to the validator dynamodb/ directory."
}
...
foreach ($p in @($WebRoot, $DynamodbRoot, $ParametersRoot, $CognitoRoot, $SesRoot, $ProxydbRoot, $PostgresdbRoot, $SecretsRoot, $SubnetsRoot, $VpcRoot, $BootstrapRoot)) {
...
function Invoke-Upstream([string]$UpstreamAction) {
    Write-Log "Upstream $UpstreamAction (via run-l3-dynamodb.ps1)"
    $env:VALIDATOR_DYNAMODB_ROOT = $DynamodbRoot
    ...
    & powershell -ExecutionPolicy Bypass -File (Join-Path $ScriptDir "run-l3-dynamodb.ps1") -Action $UpstreamAction
    if ($LASTEXITCODE -ne 0) { throw "upstream $UpstreamAction failed (exit $LASTEXITCODE)" }
}

function Invoke-WebStep([string]$WebAction, [switch]$AllowFailure) {
    $OverrideSrc = Join-Path $ScriptDir "simulith.provider-override-web.tf.example"
    $OverrideDst = Join-Path $WebRoot "simulith_override.tf"
    Copy-Item $OverrideSrc $OverrideDst -Force
    Write-Log "Copied web provider override to $OverrideDst"
    Write-Log "L3 web $WebAction start (root=$WebRoot)"
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "web init failed (exit $LASTEXITCODE)" }
        $tfvars = Join-Path $WebRoot "dev.tfvars"
        if (-not (Test-Path $tfvars)) { throw "Missing dev.tfvars: $tfvars" }
        & terraform $WebAction "-var-file=$tfvars" '-parallelism=1' '-auto-approve' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8
        if ($LASTEXITCODE -ne 0) {
            if ($AllowFailure) {
                Write-Log "WARN web $WebAction failed (exit $LASTEXITCODE) - continuing"
            } else {
                throw "web $WebAction failed (exit $LASTEXITCODE)"
            }
        }
    } finally {
    ...
    if (-not $AllowFailure -or $LASTEXITCODE -eq 0) {
        Write-Log "OK L3 web $WebAction"
    }
}

$sessionLine = "========== {0} L3 web {1} start ==========" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Action
try {
...
Write-Log "L3 web $Action start"

switch ($Action) {
    "apply" {
        Invoke-Upstream "apply"
        Invoke-WebStep "destroy" -AllowFailure
        Invoke-WebStep "apply"
    }
    "destroy" {
        Invoke-WebStep "destroy"
...
Write-Log "OK L3 web $Action complete"
