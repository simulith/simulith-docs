...
You started Simulith with Docker — **what next?** This guide is the **second step** after [quickstart](quickstart.md) and [docker](docker.md): how to work with DynamoDB, SQS, SSM, S3, Lambda, API Gateway, Secrets Manager, and EventBridge locally, and how that compares to real AWS.
...
| **Services (available)** | Full catalogs | **DynamoDB**, **SQS**, **SSM Parameter Store**, **S3**, **Lambda**, **API Gateway** (REST subset), **Secrets Manager**, **EventBridge** (schedule) — [aws-parity-overview.md](aws-parity-overview.md) |
